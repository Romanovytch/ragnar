...
def detect_llm_provider(api_base: str) -> LLMProvider:
...
def build_chat_provider_kwargs(api_base: str, reasoning_effort: str) -> dict[str, object]:
    """Return Ollama-only extensions for an OpenAI-compatible chat request."""
    if detect_llm_provider(api_base) != "ollama" or not reasoning_effort:
        return {}
...
class ChatClient:
    def __init__(
        ...
        self.provider_kwargs = build_chat_provider_kwargs(api_base, thinking)
    ...
    def complete_json(self, messages: list[dict[str, str]]) -> str:
        ...
        print(f"[debug] Sending LLM request with {len(messages)} messages and kwargs: {request_kwargs}")
